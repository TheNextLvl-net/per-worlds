...
public final class WorldBorderAdapter implements TagAdapter<WorldBorderData> {
    ...
    public WorldBorderData deserialize(final Tag tag, final TagDeserializationContext context) throws ParserException {
        ...
        final var size = root.optional("size").map(Tag::getAsDouble)
                ...
                .orElse(border.size());
        final var duration = root.optional("duration").map(tag1 -> context.deserialize(tag1, Duration.class)).orElse(border.getTransitionDuration());
        ...
        final var warningTime = root.optional("warningTime").map(tag1 -> context.deserialize(tag1, Duration.class)).orElse(border.getWarningTime());
